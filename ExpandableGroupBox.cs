...
namespace lab2
{
    // https://stackoverflow.com/questions/34631953/how-to-initialize-collapsed-groupbox-in-c
    ...
    public partial class CollapsibleGroupBox : GroupBox
    {
        ...
        void DrawToggleButton(Graphics g)
        {
            if (IsCollapsed)
            {
                Image plusImage = SystemIcons.Warning.ToBitmap();//Image.FromFile(Path.Combine(GameManager.sAssetsDir, "plus.bmp"));
                g.DrawImage(plusImage, m_toggleRect);
            }
            else
            {
                Image minusImage = SystemIcons.Asterisk.ToBitmap(); //Image.FromFile(Path.Combine(GameManager.sAssetsDir, "minus.bmp"));
